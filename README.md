...
The active dog comes from real pixel art, `skills/shiba/assets/source.png`.
`tools/build-sprite.py` is what turned it into a sprite: it finds the logical
grid in a blurry upscale by minimising the intra-cell variance, takes the
median of nine samples per cell, maps it onto the fixed palette and flood
fills the background from the edges, so the white of the chest survives.
Change the art or the expression patches and run it again:

```bash
python3 tools/build-sprite.py                # rewrites assets/sprite-full.json
```

To generate one from scratch instead, `tools/draw-sprite.py` draws a whole
...
MIT — see [LICENSE](LICENSE). The active sprite comes from original pixel
art (`skills/shiba/assets/source.png`); the alternative one is generated by
`tools/draw-sprite.py`. Both are covered by the same licence.
